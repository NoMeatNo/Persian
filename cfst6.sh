#!/bin/bash
# Ultimate Cloudflare Scanner (Final Fix)
# Combined best features: Auto-Fix, Re-Run Safe, SSL Fixed
...
function check_env() {
    ...
    # 2. Fix SSL Certificate issues (THE FIX IS HERE)
    # We add it to profile if missing...
    if ! grep -q "SSL_CERT_FILE" ~/.profile; then
        echo 'export SSL_CERT_FILE=$PREFIX/etc/tls/cert.pem' >> ~/.profile
    fi
    
    # ...BUT we export it EVERY TIME so the scanner works on re-runs
...
# Download without timeout (like the working version)
